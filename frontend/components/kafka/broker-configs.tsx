...
import { brokersApi, type BrokerConfigEntry } from "@/lib/api/brokers";
import { toast } from "./toast";
import { cn } from "@/lib/utils";

export function BrokerConfigs({ brokerId }: { brokerId: number }) {
  ...
  const configsQ = useQuery({
    queryKey: ["broker-configs", brokerId],
    queryFn: () => brokersApi.configs(brokerId),
  });

  React.useEffect(() => { if (!editing) setEdits({}); }, [editing]);

  const saveMut = useMutation({
    mutationFn: () => brokersApi.updateConfigs(brokerId, edits),
    onSuccess: (fresh) => {
      qc.setQueryData(["broker-configs", brokerId], fresh);
      setEditing(false);
      setEdits({});
      const n = Object.keys(edits).length;
      toast({ tone: "success", msg: <>Updated {n} config{n === 1 ? "" : "s"} on broker {brokerId}</> });
    },
  ...
  const total = configsQ.data?.length ?? 0;
      ...
      <div className="flex items-center gap-2 border-b border-border bg-bg-2 px-3 py-2.5">
        <div className="flex h-7 w-60 items-center gap-2 rounded-2 border border-border bg-surface px-2.5 text-fg-3 focus-within:border-accent">
          ...
          <Input
            ...
            placeholder={`Filter ${total} configs…`}
            value={filter}
        ...
        <span className="text-[11.5px] text-fg-4">{filtered.length} of {total}</span>
        <div className="flex-1" />
        {editing ? (
          <>
            <span className="text-[12px] text-fg-3">{dirtyCount} change{dirtyCount === 1 ? "" : "s"}</span>
            <Button size="sm" variant="ghost" onClick={() => { setEditing(false); setEdits({}); }} disabled={saveMut.isPending}>
              <X className="h-3 w-3" /> Cancel
            </Button>
            <Button size="sm" variant="primary" onClick={() => saveMut.mutate()} disabled={dirtyCount === 0 || saveMut.isPending}>
