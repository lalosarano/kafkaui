...
- **Java 21** — *The CI image and Docker build use JDK 21; local devs install JDK 21 (see README). The codebase relies on records and virtual threads, so 21 is the floor.*
...
- **Per-request consumer for historical reads** — opens, assigns, seeks, polls until limit or 2s, closes. Trades a few ms per request for stateless code; production deployments handling hundreds of concurrent reads should pool.
